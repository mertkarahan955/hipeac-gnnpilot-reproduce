...
The detailed per-strategy analysis across all 9 datasets, shown in Fig.~\ref{fig:strategy_basic} and Fig.~\ref{fig:strategy_advanced}, highlights how gather, scatter, and fusion strategies interact. Neighbor-packing–based kernels (kernel 1) consistently outperformed edge-based variants for sparse graphs with low average degree. Bin-packing strategies (kernels 0, 15) showed clear benefits for larger, higher-degree graphs (caidaRouterLevel, ogbn-arxiv), confirming Section 3.3's claims about improved data locality.

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures/multi_dataset_results/strategy_analysis_part1.png}
\caption{Basic optimization strategy performance across 9 datasets: (a) Gather strategies showing neighbor-packing dominance for sparse graphs, (b) Scatter strategies with node-edge variants outperforming edge-based approaches, (c) Dimension parallelization strategies (node-dim, edge-dim, node-global-dim).}
\label{fig:strategy_basic}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures/multi_datasets_results/strategy_analysis_part2.png}
\caption{Advanced strategy analysis: (a) Kernel fusion performance showing benefits of multi-operator optimization, (b) Strategy combination heatmap illustrating interaction between gather and fusion strategies, (c) Top 5 performing strategy combinations across all 9 datasets.}

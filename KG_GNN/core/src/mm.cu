...
#define KT_TILE 8
    ...
    // __shared__ float a_tile[MM_WARP_PER_BLOCK * M_TILE][K_TILE];
    ...
    for (int n_block = 0; n_block < n; n_block += N_TILE)
    {
        ...
        for (int i = 0; i < M_TILE; i++)
            for (int j = lane_id; j < N_TILE; j+=WARP_SIZE)
                c_tile[MINDEX(0, m_local_st + i, j, M_TILE_BLOCK, N_TILE)] = 0;
        ...
        for (int k_block = 0; k_block < k; k_block += K_TILE)
        {
            ...
            for (int k = 0; k < K_TILE; k += KT_TILE)
            {
                ...
                //         a_thread_tile[i][kk] = a_tile[MINDEX(dbuff_i, mt_tile_idx + i, k + kk, M_TILE_BLOCK, K_TILE)];
                // for (int kk = 0; kk < KT_TILE; kk++)
                //     for (int j = 0; j < NT_TILE; j++)
                //         //b_thread_tile[kk][j] = b_tile[(k + kk) * N_TILE + nt_tile_idx + j];
                //         b_thread_tile[kk][j] = b_tile[MINDEX(dbuff_i, k + kk, nt_tile_idx + j, K_TILE, N_TILE)];
            ...
            if (next_k_block < k)
            {
                ...
                for (int i = 0; i < M_TILE; i++)
                    ...
                    {
                        int tgt_j = j * WARP_SIZE + lane_id;
                        a_tile[MINDEX(dbuff_i_new, m_local_st + i, tgt_j, M_TILE_BLOCK, K_TILE)] = 
                ...
                for (int i = 0; i < K_TILE / MM_WARP_PER_BLOCK; i++)
                {
                    ...
                    {
                        int tgt_j = j * WARP_SIZE + lane_id;
                        b_tile[MINDEX(dbuff_i_new, tgt_i, tgt_j, K_TILE, N_TILE)] = 
        ...
        for (int i = 0; i < MT_TILE; i++)
            for (int j = 0; j < NT_TILE; j++)
                ...
                c_tile[MINDEX(0, mt_tile_idx + i, nt_tile_idx + j, M_TILE_BLOCK, N_TILE)] = c_thread_tile[i][j];

        // write back C tile
        for (int i = 0; i < M_TILE; i++)
            for (int j = lane_id; j < N_TILE; j+=WARP_SIZE)
                output_c[input_a_st * n + i * n + n_block + j] = c_tile[MINDEX(0, m_local_st + i, j, M_TILE_BLOCK, N_TILE)];
                //output_c[input_a_st * n + i * n + n_block + j] = c_thread_tile[i][j];
...
void kg_mm_execute(int m, int n, int k, float *input_a, float *input_b, float *output_c)
{
    ...
    kg_ls_ss_mm_kernel<<<block_num, thread_num, shared_size>>>(m, n, k, input_a, input_b, output_c);

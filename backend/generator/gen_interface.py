...
def generate_interface(name, fop_list, f, kid):
    ...
    f.write("fprintf(stderr, \"DEBUG {}_kernel_{:d}: info_addr=%lld, info_=%p\\n\", (long long)info_addr, (void*)info_);\n".format(name, kid))
    f.write("fflush(stderr);\n")
    f.write("if (info_ == nullptr) {\n")
    f.write("  fprintf(stderr, \"ERROR: info_ is null in {}_kernel_{:d}\\n\");\n".format(name, kid))
    f.write("  fflush(stderr);\n")
    f.write("  throw std::runtime_error(\"info_ is null in {}_kernel_{:d}\");\n".format(name, kid))
    f.write("}\n")
    f.write("fprintf(stderr, \"DEBUG {}_kernel_{:d}: Checking pointers...\\n\");\n".format(name, kid))
    f.write("fprintf(stderr, \"  rp_info=%p, rp_n=%p\\n\", (void*)info_->rp_info, (void*)info_->rp_n);\n")
    f.write("fprintf(stderr, \"  ep_info=%p, ep_n=%p\\n\", (void*)info_->ep_info, (void*)info_->ep_n);\n")
    f.write("fprintf(stderr, \"  ng_info=%p, ng_n=%p\\n\", (void*)info_->ng_info, (void*)info_->ng_n);\n")
    f.write("fflush(stderr);\n")
    ...
    f.write("  fprintf(stderr, \"ERROR: Some preprocessing pointers are null in {}_kernel_{:d}\\n\");\n".format(name, kid))
    f.write("  fflush(stderr);\n")
    f.write("  throw std::runtime_error(\"Some preprocessing pointers are null in {}_kernel_{:d}\");\n".format(name, kid))
    f.write("}\n")
    f.write("fprintf(stderr, \"DEBUG {}_kernel_{:d}: All pointers valid, proceeding...\\n\");\n".format(name, kid))
    f.write("fflush(stderr);\n")

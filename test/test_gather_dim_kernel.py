...
if ('.pt' in dataset_dir):
    ...
    #graph_csr = utils.csr_transform(graph_csr, reorder = reorder, show_degree = True)
else:
    exit
...
for i in range(3):
    ...
    func_name = "torch.ops.gatlib.gat_kernel_{:d}(info, rowptr, indices, f, fo, we, lr, e, em, es, h)".format(i)
...
if (outcsv_name):
    ...
    f.write("{:f},{:f},{:f}\n".format(perf_time_list[0], perf_time_list[1], perf_time_list[2]))
